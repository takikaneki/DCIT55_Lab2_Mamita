...
INSERT INTO tblStudent (studentNumber, firstName, lastName, middleName, birthDate, citizenship, gender) VALUES
...
UPPER (middleName) AS MiddleName FROM studentinfo;

SELECT
studentNumber, firstName, lastName, middleName, birthdate, REPLACE(citizenship, 'FILIPINO', 'Pinoy') AS citizenship, sex
FROM studentinfo;
...
SELECT
REPEAT(lastName, 5) AS RlastName
FROM studentinfo;
...
SELECT
studentNumber, firstName, lastName, middleName, DATE_FORMAT(birthdate,'%M %D %Y') AS FormatBirthdate, citizenship, sex
FROM studentinfo;
